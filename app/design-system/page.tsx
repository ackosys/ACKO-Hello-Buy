...
const GLOBAL_TABS: DSTab[] = [
  ...
  { id: 'tags', label: 'Tags & Badges', items: [
    ...
    { id: 'g-tag-plan', name: 'Plan Badges', source: 'ChatWidgets', displayMode: 'inline', render: () => <div className="flex flex-wrap gap-2">{[{ l: 'Platinum', c: 'bg-purple-500/20 text-purple-300' }, { l: 'Gold', c: 'bg-amber-500/20 text-amber-300' }, { l: 'Silver', c: 'bg-gray-500/20 text-gray-300' }, { l: 'Recommended', c: 'bg-green-500/20 text-green-300' }].map(t => <span key={t.l} className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${t.c}`}>{t.l}</span>)}</div> },
